...
def extract_title_name(text):
    ...
    # Xóa "Chương X:" hoặc "Chương X." hoặc "Chương X "
...
def format_thesis_dhyd(doc):
    ...
    for paragraph in doc.paragraphs:
        # Lấy toàn bộ chữ trong dòng để phân tích
        ...
        # Nhận diện Chương: Nếu có Style Heading 1 HOẶC bắt đầu bằng chữ "Chương"
        is_chapter = style_name == 'Heading 1' or re.match(r'^chương\s+[0-9ivx]+', full_text, re.IGNORECASE)
        # Nhận diện Mục lớn (1.1): Nếu có Style Heading 2 HOẶC bắt đầu bằng "số.số"
        is_h2 = style_name == 'Heading 2' or re.match(r'^\d+\.\d+[\.\s]', full_text)
        # Nhận diện Mục nhỏ (1.1.1): Nếu có Style Heading 3 HOẶC bắt đầu bằng "số.số.số"
        ...
        if is_chapter:
            ...
            paragraph.style = 'Heading 1' # Ép Word hiểu đây là Heading để làm Mục lục
        ...
        # --- XỬ LÝ CHỮ THƯỜNG (CÒN LẠI) ---
            ...
            for run in paragraph.runs:
                run.font.name = 'Times New Roman'
                # Ưu tiên giữ lại in đậm/in nghiêng (nếu có) của chữ thường
                run.font.size = Pt(13)
...
st.info("💡 **Mẹo:** Bạn không cần biết dùng công cụ Heading của Word. Chỉ cần gõ *'Chương 1...'* hoặc *'1.1...'* ở đầu dòng, hệ thống sẽ tự động nhận diện và tạo Mục lục cho bạn!")
    ...
    if st.button("✨ Bắt đầu chuẩn hóa định dạng", type="primary"):
        with st.spinner('Hệ thống đang phân tích và định dạng lại nội dung...'):
            try:
                ...
                st.success("🎉 Đã định dạng và nhận diện tiêu đề thông minh thành công!")
